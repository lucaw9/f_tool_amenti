...
#AutoIt3Wrapper_Icon=Resources\aibatt.png
...
Global Enum $g_eSpamButton, $g_eDefColor, $g_eSpamColor, $g_eSpamWindow, $g_eSpamInterval, $g_eSpamFKey, $g_eSpamSkill, $g_eSpamPID, $g_eSpamWindowTitle, $g_eIcon1, $g_eIcon1Path, $g_eIcon2, $g_eIcon2Path
Global $g_aSpammers[$iSpammerCount][13] ; 9 = Icon1 GUICtrl, 10 = Icon1 Path, 11 = Icon2 GUICtrl, 12 = Icon2 Path
...
#Region TABS & SPAMMERS
	...
	For $i = 0 To UBound($g_Tabs) - 1
		...
		For $j = 0 To $g_Tabs[$i][1] - 1
			...
			$g_aSpammers[$spammerNumber][$g_eSpamInterval] = GUICtrlCreateInput("0", $iLeft+$iCol1 + ($j * $iSpaceX), $iTop+64 + ($j * $iSpaceY), 60, 20, $ES_NUMBER)
			GUICtrlSetColor(-1, $COLOR_BLACK)
			GUICtrlSetBkColor(-1, $COLOR_WHITE)

			; Icon1
			$g_aSpammers[$spammerNumber][$g_eIcon1] = GUICtrlCreatePic("", $iLeft+$iCol1 + 68 + ($j * $iSpaceX), $iTop+52 + ($j * $iSpaceY), 31, 31)
			$g_aSpammers[$spammerNumber][$g_eIcon1Path] = ""
			GUICtrlSetOnEvent(-1, "_OnIconClick1")

			; Icon2
			$g_aSpammers[$spammerNumber][$g_eIcon2] = GUICtrlCreatePic("", $iLeft+$iCol1 + 104 + ($j * $iSpaceX), $iTop+52 + ($j * $iSpaceY), 31, 31)
			$g_aSpammers[$spammerNumber][$g_eIcon2Path] = ""
			GUICtrlSetOnEvent(-1, "_OnIconClick2")
	    ...
	    For $n = $g_Tabs[$i][1] To ($g_Tabs[$i][1] + $g_Tabs[$i][2]) - 1
			...
			; Edit button
			$g_aMultiPressers[$presserNumber][7] = GUICtrlCreateButton("Edit", $iLeft+$iCol2 - 7 + ($n * $iSpaceX), $iTop+37 + ($n * $iSpaceY), 44, 20)
